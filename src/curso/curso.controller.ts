...
export class CursoController {
  ...
  // CÓDIGO PARA ANEXAR UM ARQUIVO JUNTO
  /* @Post()
  ...
  async create(
    ...
    try {
      const retorno = await this.cursoService.create(createCursoDto);
      if (retorno) {
        return res.status(201).json({
          message: 'Curso cadastrado',
          data: retorno,
  ...
  @Put(':id')
  async update(@Param('id') id: number, @Body() updateCursoDto: UpdateCursoDto, @Res() res: Response) {
    try {
      const retorno = await this.cursoService.update(+id, updateCursoDto);
